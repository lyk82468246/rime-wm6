//
// rime/candidate.h -- WinCE-port mirror of upstream candidate.h.
//
// Candidate hierarchy: abstract `Candidate` + three concrete subclasses
//   * SimpleCandidate     -- holds its own text/comment/preedit strings.
//   * ShadowCandidate     -- wraps another Candidate, overriding text/comment.
//   * UniquifiedCandidate -- groups multiple equivalent Candidates.
//
// Changes vs. upstream:
//   * `= default` -> empty body.
//   * NSDMI `size_t start_ = 0;` etc. -> default-ctor mem-initialiser list.
//   * `using CandidateQueue = list<of<Candidate>>;` -> typedef + `> >` to
//     avoid MSVC 9.0's `>>` parse.
//   * Same `> >` fix in `vector<of<Candidate> >`.
//
...
class Candidate {
 public:
  Candidate() : start_(0), end_(0), quality_(0.) {}
...
typedef list<of<Candidate> > CandidateQueue;
typedef vector<of<Candidate> > CandidateList;
