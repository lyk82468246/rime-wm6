//
// rime/context.cc -- WinCE-port mirror of upstream context.cc.
//
// Changes vs. upstream:
//   * `auto cand = ...` / `auto it = ...` -> explicit types.
//   * `set_composition(Composition&&)` rvalue overload -> a const-ref copy
//     overload that assigns (we pay one segment-vector copy; comp is
//     small).
//   * `nullptr` -> default `an<T>()`.
//   * `composition_.rbegin()` / `rend()` -- reverse_iterator works in
//     C++03 since std::vector / Segmentation inherits the typedefs.
//
#include <algorithm>
#include <utility>
...
// U+2038 CARET, encoded as UTF-8. Hex-escape keeps the source file ASCII-clean
// (this is what the upstream kCaretSymbol literal expands to byte-for-byte).
...
bool Context::HasMenu() const {
  if (composition_.empty())
    return false;
  const an<Menu>& menu(composition_.back().menu);
...
bool Context::Select(size_t index) {
  ...
  an<Candidate> cand = seg.GetCandidateAt(index);
  if (cand) {
...
void Context::set_composition(const Composition& comp) {
  // We pay one segment-vector copy here in place of upstream's std::move.
  composition_ = comp;
}
...
bool Context::get_option(const string& name) const {
  map<string, bool>::const_iterator it = options_.find(name);
  if (it != options_.end())
    return it->second;
  else
    return false;
...
string Context::get_property(const string& name) const {
  map<string, string>::const_iterator it = properties_.find(name);
...
void Context::ClearTransientOptions() {
  map<string, bool>::iterator opt = options_.lower_bound("_");
  while (opt != options_.end() && !opt->first.empty() && opt->first[0] == '_') {
    options_.erase(opt++);
  }
  map<string, string>::iterator prop = properties_.lower_bound("_");
