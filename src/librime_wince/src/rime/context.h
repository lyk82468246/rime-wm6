//
// rime/context.h -- WinCE-port mirror of upstream context.h.
//
// Owns the active Composition + CommitHistory + options/properties for one
// editing session. Notifies its signal<> members when state changes; the
// engine and gear/ processors wire callbacks in.
//
// Changes vs. upstream:
//   * `using Notifier = signal<void(Context*)>` template aliases ->
//     typedef.
//   * `= default` -> empty body.
//   * NSDMI `size_t caret_pos_ = 0;` -> default-ctor mem-init list.
//   * `void set_composition(Composition&&)` rvalue-ref overload ->
//     `void set_composition(const Composition&)`. We pay one O(n) copy
//     (n = segment count, typically <10) instead of an unavailable move.
//     If profiling shows this matters later, switch to a swap-in-place
//     overload that takes a non-const reference.
//
...
class RIME_DLL Context {
 public:
  ...
  typedef signal<void(Context* ctx, const string& option)> OptionUpdateNotifier;
  typedef signal<void(Context* ctx, const string& property)>
      PropertyUpdateNotifier;
  typedef signal<void(Context* ctx, const KeyEvent& key_event)>
      KeyEventNotifier;

  Context() : caret_pos_(0) {}
  ~Context() {}
  ...
  void set_composition(const Composition& comp);
