//
// rime/key_event.cc -- WinCE-port mirror of upstream key_event.cc.
//
// Changes vs. upstream:
//   * `auto it = cbegin();` -> explicit `const_iterator` (C++03 has no auto).
//   * No other changes; the no_logging.h stub already neutralises LOG().
//
#include <sstream>
#include <iomanip>
...
KeyEvent::KeyEvent(const string& repr) : keycode_(0), modifier_(0) {
  if (!Parse(repr))
...
string KeySequence::repr() const {
  ...
  string k;
  for (KeySequence::const_iterator it = begin(); it != end(); ++it) {
    k = it->repr();
    if (k.size() == 1) {
      result << k;
