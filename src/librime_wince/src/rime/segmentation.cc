//
// rime/segmentation.cc -- WinCE-port mirror of upstream segmentation.cc.
//
// Changes vs. upstream:
//   * `auto cand = ...` -> explicit an<Candidate>.
//   * `nullptr` -> default an<Candidate>().
//   * Three range-for loops -> classic iterator loops.
//
#include <algorithm>
#include <iterator>
#include <ostream>
...
void Segment::Close() {
  an<Candidate> cand = GetSelectedCandidate();
...
bool Segmentation::AddSegment(Segment segment) {
  size_t start_pos = GetCurrentStartPosition();
  if (segment.start != start_pos) {
...
bool Segmentation::Forward() {
  ...
  push_back(Segment(static_cast<int>(back().end),
                    static_cast<int>(back().end)));
  return true;
}
...
size_t Segmentation::GetConfirmedPosition() const {
  size_t k = 0;
  for (Segmentation::const_iterator it = begin(); it != end(); ++it) {
...
std::ostream& operator<<(std::ostream& out, const Segmentation& segmentation) {
  ...
  for (Segmentation::const_iterator sit = segmentation.begin();
    ...
    if (!segment.tags.empty()) {
      out << "{";
      bool first = true;
      for (set<string>::const_iterator tit = segment.tags.begin();
           tit != segment.tags.end(); ++tit) {
        if (first)
          first = false;
        else
          out << ",";
